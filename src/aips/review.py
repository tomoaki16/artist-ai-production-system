...
.hidden{{display:none!important}}.brief-card{{background:white;border:1px solid var(--line);border-radius:14px;padding:20px;margin:14px 0}}.brief-card label{{display:block;font-weight:750;margin:0 0 8px}}.brief-card label span{{display:block;color:var(--sub);font-size:13px;font-weight:400;margin-top:3px}}textarea{{width:100%;min-height:110px;resize:vertical;border:1px solid var(--line);border-radius:10px;padding:12px;font:inherit;line-height:1.55}}textarea:focus,select:focus{{border-color:var(--accent);outline:none}}select{{font:inherit;border:1px solid var(--line);border-radius:9px;padding:9px;background:white}}.scope{{display:grid;grid-template-columns:1fr 1fr;gap:10px}}.scope div{{background:var(--soft);border-radius:12px;padding:14px}}.scope b{{display:block;font-size:22px}}.secondary{{border:1px solid var(--line);border-radius:10px;background:white;color:var(--ink);padding:10px 14px;font-weight:700;cursor:pointer}}.footer-actions{{display:flex;gap:8px}}
</style></head><body><main><section id="analysis-step"><h1>曲を読む</h1><p class="lead">解析が違う場合は直接修正し、コード・ベース・ギターごとにAIが提案してよい範囲を決めます。</p><section class="summary"><div><b>{len(rows)}</b><span>解析済み小節</span></div><div><b id="corrected-count">0</b><span>修正した項目</span></div><div><b id="open-count">0</b><span>提案可の項目</span></div></section><section>{body}</section><details><summary>元の解析データ</summary><pre>{raw}</pre></details></section>
<section id="brief-step" class="hidden"><h1>Producerに依頼する</h1><p class="lead">作品をどうしたいかはArtistが決めます。AIは開放された範囲だけで方法を考えます。</p><div class="scope"><div><b id="brief-protected">0</b><span>固定する項目</span></div><div><b id="brief-open">0</b><span>AIが提案できる項目</span></div></div>
<div class="brief-card"><label for="artist-intent">どうしたい？<span>表現したいこと、聴かせ方、感覚を自分の言葉で入力</span></label><textarea id="artist-intent" placeholder="例：13〜16小節でサビへの期待感を強めたい。ただし大げさにはしたくない。"></textarea></div>
<div class="brief-card"><label for="artist-constraints">必ず守ってほしいこと<span>1行に1つ。固定した音楽情報に加える条件</span></label><textarea id="artist-constraints" placeholder="例：\nギターの音数は増やしすぎない\nサビ直前の間は残す"></textarea></div>
<div class="brief-card"><label for="proposal-count">何案比較する？</label><select id="proposal-count"><option value="2">2案</option><option value="3" selected>3案</option><option value="4">4案</option><option value="5">5案</option></select></div></section></main>
<footer><div class="footer-inner"><span id="decision-summary">すべて固定・修正なし</span><div class="footer-actions"><button id="back" class="secondary hidden">戻る</button><button id="next" class="primary">解析内容を確定して次へ</button></div></div></footer><script>
...
let confirmed=null;function collectConfirmed(){{const inputs=[...document.querySelectorAll('.analysis-value')];const empty=inputs.filter(input=>!input.value.trim());inputs.forEach(input=>input.classList.toggle('error',!input.value.trim()));if(empty.length){{empty[0].focus();alert('空欄があります。解析結果を入力してください。');return null;}}const decisions=[...document.querySelectorAll('.bar-row')].map(card=>({{bar:Number(card.dataset.bar),parts:Object.fromEntries([...card.querySelectorAll('.part')].map(part=>{{const value=part.querySelector('.analysis-value').value.trim(),original=part.dataset.original;return [part.dataset.part,{{value,original_value:original,corrected:value!==original,authority:part.querySelector('.active').dataset.value}}];}}))}}));return {{schema_version:'0.2',confirmed_at:new Date().toISOString(),bar_decisions:decisions}};}}
function producerRequest(){{const intent=document.querySelector('#artist-intent').value.trim();if(!intent){{document.querySelector('#artist-intent').focus();alert('「どうしたい？」を入力してください。');return null;}}const constraints=document.querySelector('#artist-constraints').value.split('\\n').map(x=>x.trim()).filter(Boolean),protectedItems=[],editableItems=[],bars=confirmed.bar_decisions.map(item=>{{const context={{}},authority={{}};Object.entries(item.parts).forEach(([name,part])=>{{context[name]=part.value;authority[name]=part.authority;const target={{bar:item.bar,part:name,value:part.value}};(part.authority==='protect'?protectedItems:editableItems).push(target);}});return {{bar:item.bar,confirmed_context:context,authority}};}});return {{schema_version:'0.1',purpose:'producer_arrangement_proposals',artist:{{intent,constraints,owns_taste_and_final_decision:true}},music_context:{{bars,source:'artist_confirmed_analysis'}},authority_boundary:{{protected:protectedItems,editable:editableItems,rule:'Never alter protected values. Propose changes only for editable parts.'}},requested_output:{{proposal_count:Number(document.querySelector('#proposal-count').value),return_multiple_options:true,include_short_rationale:true,format:'structured_music_proposals'}}}};}}
...
document.querySelector('#next').onclick=()=>{{if(!document.querySelector('#analysis-step').classList.contains('hidden')){{confirmed=collectConfirmed();if(!confirmed)return;const open=confirmed.bar_decisions.flatMap(x=>Object.values(x.parts)).filter(x=>x.authority==='open').length;document.querySelector('#brief-open').textContent=open;document.querySelector('#brief-protected').textContent=confirmed.bar_decisions.length*3-open;document.querySelector('#analysis-step').classList.add('hidden');document.querySelector('#brief-step').classList.remove('hidden');document.querySelector('#back').classList.remove('hidden');document.querySelector('#next').textContent='AIへの依頼内容を確定';document.querySelector('#decision-summary').textContent='ステップ2 / 2・Artistの意図';window.scrollTo(0,0);}}else{{const request=producerRequest();if(request)downloadJson(request,'producer-request.json');}}}};
